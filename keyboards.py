...
EXPENSE_CATEGORIES = ["Еда", "Каршеринг", "Метро", "Аренда", "Связь",
                      "Долги/кредиты", "Разработка", "Озон", "Прочее"]
...
def expense_categories() -> InlineKeyboardMarkup:
    ...
    for c in EXPENSE_CATEGORIES:
        b.button(text=c, callback_data=f"exp_cat:{c}")
    b.button(text="« Назад", callback_data="home")
    b.adjust(2, 2, 2, 2, 1)
    return b.as_markup()
...
def category_for_remain() -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    for c in EXPENSE_CATEGORIES:
        b.button(text=c, callback_data=f"rem_cat:{c}")
    b.button(text="« Назад", callback_data="home")
    b.adjust(2, 2, 2, 2, 1)
    return b.as_markup()
...
def debts_to_pay(debts: list) -> InlineKeyboardMarkup:
    """Кнопки с долгами (остаток>0) для выбора что гасить."""
    b = InlineKeyboardBuilder()
    for d in debts:
...
def skip_or_back() -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.button(text="Пропустить", callback_data="skip_note")
    b.button(text="« Отмена", callback_data="home")
    b.adjust(2)
    return b.as_markup()
